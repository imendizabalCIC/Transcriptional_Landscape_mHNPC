...
receiver_v <- c("9-Mast cell", "10-Macrophage", "11-T cell", "13-Endothelial", "14-Fibroblast", "16-Endothelial", "17-T cell", "18-Fibroblast", "19-B Cell")

for (r in 1:length(receiver_v)){
  
  tag_niche <- paste("6_Luminal_to_",receiver_v[r],sep='')
  niches = list( # user adaptation required on own dataset
  "mHNPC_niche" = list(
   "sender" = c("6_Luminal_mHNPC"),
   "receiver" = paste(receiver_v[r],"_mHNPC",sep="")),
  "LPCa_niche" = list(
   "sender" = c("6_Luminal_LPCa"),
   "receiver" = paste(receiver_v[r],"_LPCa",sep=""))) 
     
  ####################################################################
  ## Read in the expression data
  ####################################################################
  annotated <- readRDS("Annotation_Manual_Discovery.rds")

  vector <- as.character(annotated$annotation)
  vector[which(vector =="9-Mast cell")] <- "9_Mast_cell"
  vector[which(vector =="10-Macrophage")] <- "10_Macrophage"
  vector[which(vector =="11-T cell")] <- "11_T_cell"
  vector[which(vector =="13-Endothelial")] <- "13_Endothelial"
  vector[which(vector =="14-Fibroblast")] <- "14_Fibroblast"
  vector[which(vector =="16-Endothelial")] <- "16_Endothelial"
  vector[which(vector =="17-T cell")] <- "17_T_cell"
  vector[which(vector =="18-Fibroblast")] <- "18_Fibroblast"
  vector[which(vector =="19-B Cell")] <- "19_B_Cell"
  annotated$annotation <- vector
  
  # I will remove the cells belonging to other categories other than localized and mHNPC
  seurat_obj <- subset(x = annotated, subset = class %in% c("LPCa","mHNPC"))
  
  table(seurat_obj$class)
  
  DimPlot(seurat_obj, group.by = "annotation") # "celltype" in the tutorial
  DimPlot(seurat_obj, group.by = "class") #"pEMT" in the dataset

  # We will now also check the number of cells per cell type condition combination
  table(seurat_obj@meta.data$annotation, seurat_obj@meta.data$class) # cell types vs conditions 
  ...
  seurat_obj@meta.data$celltype_aggregate = paste(seurat_obj@meta.data$annotation, seurat_obj@meta.data$class, sep = "_") # user adaptation required on own dataset
