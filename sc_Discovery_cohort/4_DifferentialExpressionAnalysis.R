# Groups of interest
data_annotated$class <- NA
data_annotated$class[grep("P_N", data_annotated$pheno)] <- "LPCa"
data_annotated$class[grep("P$", data_annotated$pheno)] <- "LPCa"
data_annotated$class[grep("P_B", data_annotated$pheno)] <- "mHNPC"
data_annotated$class[grep("P_M", data_annotated$pheno)] <- "Met"

###################################################################
## Differential expression testing - pseudobulk DESeq2
###################################################################
data_annotated$class <- as.factor(data_annotated$class)
...
class_missing <- as.character(table_singlet$Var2[ind_missing])

groups <- data_singlet@meta.data[, c("seurat_clusters", "class", "orig.ident")]
...
if (length(ind_missing) >= 1) {
  ...
  for (i in 1:length(class_missing)){
    ...
    colnames(matrix_counts)[dim(matrix_counts)[2]] <-  paste(class_missing[i], "_", class_missing[i], "_FAKE-", cluster_missing[i],sep='')
    # For aggregation, we use Matrix.utils's aggregate. Matrix function with the colData columns "cluster_id" and "class" as groupings. Note that aggregate.Matrix initially yields a matrix of dimensions #(cluster-sample-instances) ? #(genes), which we re-split and transform to obtain, for each cluster, and matrix of dimensions #(genes) ? #(samples):
    # aggregate by cluster-sample
    group_missing <- as.character(groups$class[grep(class_missing[i],groups$orig.ident)[1]])
    new_row <- c(cluster_missing[i],group_missing,class_missing[i])
    groups <- rbind.data.frame(groups,new_row)
    #    row.names(groups)[dim(groups)[1]] <- paste(colnames(matrix_counts)[dim(matrix_counts)[2]],  sep ="")
...
class_names <- length(phen <- purrr::set_names(levels(groups$class)))
...
# The approach used here for aggregation presents but one of many ways, and was selected due to its simplicity and efficiency. Alternatively, one could, for example, split the character vector of cells (colnames(sce)) by cluster-class, and use rowSums(counts(sce[, ...])) for aggregation. However, this approach is comparatively more verbose and less efficient:
# split cell by cluster-class
...
gg_df <- left_join(gg_df, ei[, c("orig.ident", "class")]) 
metadata <- gg_df %>%   dplyr::select(seurat_clusters, orig.ident, class) 
...
if (length(ind_missing) >= 1) {
  for(m in 1:length(cluster_missing)) {
    pb2[[cluster_missing[m]]][grep("\\PTEN\\b",row.names(pb2[[cluster_missing[m]]])), which(colnames(pb2[[cluster_missing[m]]]) == class_missing[m])] <- 0
  }
} 
...
p_thres <- 0.1
de_metadata <- metadata[which(metadata$class %in% c("LPCa","mHNPC")),]
...
for(i in 1:length(clusters)) {
  ...
  dds <- DESeqDataSetFromMatrix(cluster_counts, colData = cluster_metadata, design = ~ class )
  ...
  rld_cor <- cor(rld_mat)
  # Plot heatmap
  pheatmap(rld_cor, annotation = cluster_metadata[, c("class"), drop=F])
  ...
  levels(cluster_metadata$class)[3]
  
  contrast <- c("class", levels(cluster_metadata$class)[1], levels(cluster_metadata$class)[3])
  # /////////////////////////////////////////////////////////////////
